...
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize, frame=single, backgroundcolor=\color{gray!10}, breaklines=true, caption={Docker Compose Configuration}]
services:
  db:
    image: postgres:15
    environment:
...
\begin{lstlisting}[basicstyle=\ttfamily\small, frame=single, backgroundcolor=\color{gray!10}, breaklines=true]
healthcheck:
  test: ["CMD-SHELL", "pg_isready -U $POSTGRES_USER"]
  interval: 5s
  timeout: 5s
...
\begin{lstlisting}[basicstyle=\ttfamily\small, frame=single, backgroundcolor=\color{gray!10}, breaklines=true, caption={Deployment Commands}]
...
The complete source code is available at: \url{https://github.com/cagataydisli/BLG317_Proje}
